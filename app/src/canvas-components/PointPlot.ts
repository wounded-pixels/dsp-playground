...
export class PointPlot {
  ...
  drawXTickLabel = (xValue: number) => {
    this.ctx.fillText(
      '' + xValue,
      this.translateX(xValue) - 5,
      this.topPadding + this.calculatePlotHeight() + this.textHeight + 8
    );
  };

  drawYTickLabel = (yValue: number) => {
    this.ctx.fillText(
      yValue.toString().padStart(2, ' '),
      this.textHeight - 8,
      this.translateY(yValue) + this.textHeight / 2
    );
  };
  ...
  plotPoints = (values: any[]) => {
    ...
    this.ctx.beginPath();
    this.ctx.moveTo(this.translateX(initialX), this.translateY(initialY));
